...
class Config(object):
    ...
    MONGODB_SETTINGS = {'DB': 'OctBlog'}   # mongodb数据库设置

    TEMPLATE_PATH = os.path.join(BASE_DIR, 'templates').replace('\\', '/')  # 模板路径
    STATIC_PATH = os.path.join(BASE_DIR, 'static').replace('\\', '/')       # 静态文件路径
    EXPORT_PATH = os.path.join(BASE_DIR, 'exports').replace('\\', '/')

    if not os.path.exists(EXPORT_PATH):
        os.makedirs(EXPORT_PATH)

    REMEMBER_COOKIE_DURATION = datetime.timedelta(hours=3)  # cookie数据的有效期
...
config = {
    'dev': DevConfig,   # 开发模式
    'prd': PrdConfig,   # 生产/线上模式
    'testing': TestingConfig,  # 测试模式
    'default': DevConfig,   # 开发模式
}
